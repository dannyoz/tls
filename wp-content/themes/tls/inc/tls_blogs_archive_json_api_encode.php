...
function tls_blogs_archive_json_api_encode( $response )
{
    ...
    if ( isset( $response[ 'page' ] ) && $response[ 'page_template_slug' ] == 'template-blogs-archive.php' ) {
        ...
        $response[ 'posts' ][ ] = array( 'type' => 'listen_blog', 'id' => $spotlight_podcast->ID, 'title' => $spotlight_podcast->post_title, 'excerpt' => tls_make_post_excerpt( $spotlight_podcast, 15 ), 'link' => get_permalink( $spotlight_podcast->ID ), 'soundcloud' => $spotlight_podcast_custom_fields[ 'soundcloud_embed_code' ][ 0 ] );
        ...
        $blogs_archive_args = array( 'post_type' => array( 'post' ), 'orderby' => 'date', 'order' => 'DESC', 'paged' => $paged, 'post__not_in' => array( $featured_post->ID, $spotlight_podcast->ID ), );
