...
class TlsTealium
{
    ...
    function __construct()
    {
        ...
        add_action( 'wp_head', array( $this, 'tls_tealium_head' ) );
    }
    ...
    public function tls_tealium_head()
    {
        ...
        if ( is_front_page() ) { // Front Page (Home Page)
        ...
        } else if ( is_page_template( 'template-latest-edition.php' ) ) { // Latest Edition Page
            ...
            $tealium_data = $this->tealium_data( 'latest editions:' . $latest_edition->post_title, 'section level 1', 'latest editions' );

        } else if ( is_singular( 'tls_editions' ) ) { // Single Page for Editions
            ...
            $tealium_data = $this->tealium_data( 'latest editions:' . $edition_title, 'section level 1', 'latest editions' );

        } else if ( is_page_template( 'template-articles-archive.php' ) ) { // Discover Page

            // Article Archive (Discover Page) Tealium Data Object
            $tealium_data = $this->tealium_data( 'discover', 'section level 1', 'discover' );

        } else if ( is_page_template( 'template-blogs-archive.php' ) ) { // Blogs Page

            // Blogs Archive Tealium Data Object
            $tealium_data = $this->tealium_data( 'blogs', 'section level 1', 'blogs' );

        } else if ( is_singular( 'post' ) ) { // Single Blog Page
            ...
            $tealium_data = $this->tealium_data( 'blog post:' . $blog_title, 'blog post', 'blogs', 'public', array( 'page_section_2' => 'blogs:' . $blog_category[ 0 ]->name ) );

        } else if ( is_search() ) { // Search Page

            global $wp_query; // Get the Global $wp_query variable
            // Search Page Tealium Data Object
            $tealium_data = $this->tealium_data( 'search results', 'search', 'search', 'public', array( 'event_internal_search' => 'internal search submitted', 'internal_search_term' => $wp_query->query_vars[ 's' ], 'internal_search_results' => $wp_query->found_posts, 'page_number' => "1 of " . $wp_query->max_num_pages ) );

        } else if ( is_tax( 'article_section' ) ) { // Article Section (Article Category) Archive Page

            $article_section = get_term_by( 'slug', get_query_var( 'term' ), get_query_var( 'taxonomy' ) ); // Get the Current Section Term being queried
            // Article Section Archive Tealium Data Object
            $tealium_data = $this->tealium_data( html_entity_decode( $article_section->name ), 'section level 2', 'info and navigation' );

        } else if ( is_category() ) { // Blog Post Category Archive

            $category = get_queried_object();
            // Blog Category Archive Tealium Data Object
            $tealium_data = $this->tealium_data( 'blog:' . html_entity_decode( $category->name ), 'blog', 'blogs', 'public', array( 'page_section_2' => 'blogs:' . html_entity_decode( $category->name ) ) );
            ...
            $tealium_data = $this->tealium_data( $article_id . ':' . $article->post_title, 'article:' . $page_type, html_entity_decode( $article_section[ 0 ]->name ), $page_restriction, array( 'article_name' => $article->post_title, 'article_publish_timestamp' => strtolower( date( 'Y/m/d H:i l', strtotime( $article->post_date ) ) ), 'article_id' => $article_id, 'article_author' => ( isset( $article_custom[ 'article_author_name' ][ 0 ] ) ) ? $article_custom[ 'article_author_name' ][ 0 ] : '', ) );

        } else if ( is_page_template( 'template-faqs.php' ) ) { // FAQs Page

            // FAQs Page Tealium Data Object
            $tealium_data = $this->tealium_data( 'frequently asked questions', 'info and navigation', 'info and navigation' );

        } else if ( is_page_template( 'template-page-with-accordion.php' ) ) { // if it is a page with the Accordion Page template
            ...
            if ( strtolower( $page->post_title ) == 'contact us' ) { // if it is contact us page
                ...
                $tealium_data = $this->tealium_data( 'contact us', 'info and navigation', 'info and navigation' );

            } else if ( strtolower( $page->post_name ) == 'terms-conditions' || strtolower( $page->post_name ) == 'terms-and-conditions' ) { // If the current page is Terms & Conditions

                // Terms & Conditions Page Tealium Page Data Object
                $tealium_data = $this->tealium_data( 'terms and conditions', 'info and navigation', 'info and navigation' );

            }

        } else if ( is_404() ) { // if it is a 404 Page

            // 404 Page Tealium Data Object
            $tealium_data = $this->tealium_data( '', 'errors', 'errors', 'public', array( 'error_type' => 'page not found', 'error_name' => 'error' ) );

        } else if ( is_singular( 'page' ) ) { // If it is single page
            ...
            if ( strtolower( $page->post_title ) == 'how to advertise' ) { // If the Current Page is How to Advertise
                ...
                $tealium_data = $this->tealium_data( 'how to advertise', 'info and navigation', 'info and navigation' );
     ...
     */
    function tealium_data( $page_name = null, $page_type = null, $page_section = null, $page_restrictions = 'public', $extra_options = array() )
    {
        $tealium_data = array( 'page_name' => $page_name, 'page_type' => $page_type, 'page_section' => $page_section, 'page_restrictions' => $page_restrictions, );
