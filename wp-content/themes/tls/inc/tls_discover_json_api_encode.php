...
function tls_discover_json_api_encode( $response )
{
    ...
    if ( isset( $response[ 'page' ] ) && $response[ 'page_template_slug' ] == 'template-articles-archive.php' ) {
        ...
        $response[ 'top_articles' ][ ] = array( 'spotlight' => true, 'type' => 'article', 'id' => $spotlight_article->ID, 'url' => get_permalink( $spotlight_article->ID ), 'title' => $spotlight_article->post_title, 'excerpt' => tls_make_post_excerpt( $spotlight_article, 15 ), 'author' => array( 'name' => get_the_author_meta( 'display_name', $spotlight_article->post_author ), 'slug' => get_the_author_meta( 'slug', $spotlight_article->post_author ), ), 'custom_fields' => array( 'thumbnail_image_url' => $spotlight_article_thumbnail, 'teaser_summary' => ( $spotlight_article_teaser ) ? $spotlight_article_teaser : tls_make_post_excerpt( $spotlight_article, 15 ), ), 'taxonomy_article_section' => $spotlight_article_sections, 'taxonomy_article_section_url' => get_term_link( $spotlight_article_sections[ 0 ]->term_id, $spotlight_article_sections[ 0 ]->taxonomy ), 'taxonomy_article_visibility' => $spotlight_article_visibility, 'books' => get_field( 'books', $spotlight_article->ID ), );
        ...
        foreach ( $article_sections as $section ) {
            ...
            if ( $section->term_id != $spotlight_article_section ) {
                ...
                // Add this article into top_articles array from the JSON Response Object
                $response[ 'top_articles' ][ ] = array( 'type' => 'article', 'id' => $top_section_article->ID, 'url' => get_permalink( $top_section_article->ID ), 'title' => $top_section_article->post_title, 'excerpt' => tls_make_post_excerpt( $top_section_article, 15 ), 'author' => array( 'name' => get_the_author_meta( 'display_name', $top_section_article->post_author ), 'slug' => get_the_author_meta( 'slug', $top_section_article->post_author ), ), 'custom_fields' => array( 'thumbnail_image_url' => $top_section_article_thumbnail, 'teaser_summary' => ( $top_section_article_teaser ) ? $top_section_article_teaser : tls_make_post_excerpt( $top_section_article, 15 ), ), 'taxonomy_article_section' => $top_section_article_section, 'taxonomy_article_section_url' => get_term_link( $top_section_article_section[ 0 ]->term_id, $top_section_article_section[ 0 ]->taxonomy ), 'taxonomy_article_visibility' => $top_section_article_visibility, 'books' => get_field( 'field_54edde1e60d80', $top_section_article->ID ), );
        ...
        // WP_Query Arguments
        $articles_archive_args = array( 'post_type' => array( 'tls_articles' ), 'orderby' => 'date', 'order' => 'DESC', 'paged' => $paged, 'post__not_in' => $top_section_articles, // Array of ID's of Top Articles to remove
            'tax_query' => array( array( 'taxonomy' => 'article_section', 'field' => 'term_id', 'terms' => $show_sections_ids ) ) );
