...
function tls_search_results_json_api_encode( $response )
{
    ...
    if ( isset( $response[ 'posts' ] ) && $wp_query->is_search == true ) {
        ...
        if ( isset( $_GET[ 'date_filter' ] ) && $_GET[ 'date_filter' ] != '' ) {
            ...
            $date_posts_archive_args = array( 'date_query' => array( array( 'column' => 'post_date', 'after' => $url_query[ 'date_filter' ], 'inclusive' => true ), ), );
        ...
        $response[ 'content_type_filters' ][ '2_public_articles' ] = array( 'item_label' => __( 'Free To Non Subscribers', 'tls' ), 'json_query' => 'post_type=tls_articles&article_visibility=public', 'search_count' => (int) $public_articles->found_posts );
        ...
        $response[ 'content_type_filters' ][ '1_reviews' ] = array( 'item_label' => __( 'Reviews', 'tls' ), 'json_query' => 'post_type=tls_articles', 'search_count' => ( $current_query[ 'post_type' ] == 'tls_articles' || $wp_query->query_vars[ 'post_type' ] == 'any' ) ? $reviews->found_posts : 0, );
        ...
        $response[ 'content_type_filters' ][ '3_blogs' ] = array( 'item_label' => __( 'Blogs', 'tls' ), 'json_query' => 'post_type=post', 'search_count' => (int) ( $current_query[ 'post_type' ] == 'post' || $wp_query->query_vars[ 'post_type' ] == 'any' ) ? $blogs->found_posts : 0, );
        ...
        $response[ 'content_type_filters' ][ '4_faqs' ] = array( 'item_label' => __( 'FAQs', 'tls' ), 'json_query' => 'post_type=tls_faq', 'search_count' => (int) ( $current_query[ 'post_type' ] == 'tls_faq' || $wp_query->query_vars[ 'post_type' ] == 'any' ) ? $faqs->found_posts : 0, );
         ...
         */
        // Past 7 Days
        $past_7_days_args = array( 'date_query' => array( array( 'column' => 'post_date', 'after' => '7 days ago', 'inclusive' => true ), ), );
        $past_7_days_query = array_merge( (array) $current_query, $past_7_days_args );
        $past_7_days = new WP_Query( $past_7_days_query );
        wp_reset_query();
        $response[ 'date_filters' ][ '1_past_7_days' ] = array( 'item_label' => __( 'Past 7 Days' ), 'search_term' => '7 days ago', 'search_count' => (int) $past_7_days->found_posts );

        // Past 30 Days
        $past_30_days_args = array( 'date_query' => array( array( 'column' => 'post_date', 'after' => '30 days ago', 'inclusive' => true ), ), );
        $past_30_days_query = array_merge( (array) $current_query, $past_30_days_args );
        $past_30_days = new WP_Query( $past_30_days_query );
        wp_reset_query();
        $response[ 'date_filters' ][ '2_past_30_days' ] = array( 'item_label' => __( 'Past 30 Days' ), 'search_term' => '30 days ago', 'search_count' => (int) $past_30_days->found_posts );

        // Past 1 Year
        $past_1_year_args = array( 'date_query' => array( array( 'column' => 'post_date', 'after' => '1 year ago', 'inclusive' => true ), ), );
        $past_1_year_query = array_merge( (array) $current_query, $past_1_year_args );
        $past_1_year = new WP_Query( $past_1_year_query );
        wp_reset_query();
        $response[ 'date_filters' ][ '3_past_1_year' ] = array( 'item_label' => __( 'Past Year' ), 'search_term' => '1 year ago', 'search_count' => (int) $past_1_year->found_posts );

        // Past 5 Years
        $past_5_years_args = array( 'date_query' => array( array( 'column' => 'post_date', 'after' => '5 years ago', 'inclusive' => true ), ), );
        $past_5_years_query = array_merge( (array) $current_query, $past_5_years_args );
        $past_5_years = new WP_Query( $past_5_years_query );
        wp_reset_query();
        $response[ 'date_filters' ][ '4_past_5_years' ] = array( 'item_label' => __( 'Past 5 Years' ), 'search_term' => '5 years ago', 'search_count' => (int) $past_5_years->found_posts );

        // Past 10 Years
        $past_10_years_args = array( 'date_query' => array( array( 'column' => 'post_date', 'after' => '10 years ago', 'inclusive' => true ), ), );
        $past_10_years_query = array_merge( (array) $current_query, $past_10_years_args );
        $past_10_years = new WP_Query( $past_10_years_query );
        wp_reset_query();
        $response[ 'date_filters' ][ '5_past_10_years' ] = array( 'item_label' => __( 'Past 10 Years' ), 'search_term' => '10 years ago', 'search_count' => (int) $past_10_years->found_posts );
        ...
        foreach ( $sections as $key => $value ) {
            ...
            $response[ 'articles_sections' ][ $value->slug ] = array( 'item_label' => $value->name, 'type' => 'taxonomy', 'json_query' => $value->slug, 'taxonomy' => $value->taxonomy, 'slug' => $value->slug, 'search_count' => $sections_count->found_posts );
        }
