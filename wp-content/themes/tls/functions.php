...
function clear_log_error_messages()
{
    ...
    // Get Current Date
    $carbon_date_now = new \Carbon\Carbon();

    // Clear Log & Error Messages
    $current_options['log_messages'] = $current_options['log_messages'] . "\n" . $carbon_date_now;
    $current_options['error_messages'] = $current_options['error_messages'] . "\n" . $carbon_date_now;
...
function activate_clear_logs_schedule() {
    if ( !wp_next_scheduled( 'clear_log_error_messages_hook' ) ) {
        wp_schedule_event(current_time('timestamp'),'every_15_mins','clear_log_error_messages_hook');
    }
}
